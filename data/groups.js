...
const createGroup = async (groupLeaderId, groupName) => {
  ...
  let leader = await users.getUserById(parsedLeaderId.toString());
  leader.userGroups.push(stringId);
  users.updateUser(parsedLeaderId.toString(), leader);
  return group;
};
...
const addGroupMember = async (groupId, userId) => {
  ...
  if (updateInfo.modifiedCount === 0)
    throw new Error("Could not add group member");
  //Update user's groups
  let user = await users.getUserById("" + parsedUserId);
  user.userGroups.push(groupId);
  users.updateUser(parsedUserId.toString(), user);
...
const deleteGroup = async (groupId) => {
  ...
  const deleted = await groupCollection.findOneAndDelete({
    _id: parsedGroupId,
  });
...
const createSession = async (groupId, voteCountNeeded, filters) => {
  ...
  let decisionGroup = await getGroupById(parsedGroupId.toString());
  if (!decisionGroup) {
  ...
  movieList = updateWatchList(
    groupId,
    movieList,
    decisionGroup.groupLeaderId.toString()
  );

  if (voteCountNeeded > decisionGroup.groupMembers.length) {
    throw new Error(
      "vote count must be less than current total of group members"
    );
  }
  let newSession = {
  ...
  return await groupCollection.getGroupById(parsedGroupId.toString());
};
  ...
  for (let item of group.currentSession.movieList) {
    if (item.movie.toString() == movieId.toString()) {
      ...
      if (item.votes == group.currentSession.voteCountNeeded) {
        await pastSessions.createPastSession(
          groupId,
